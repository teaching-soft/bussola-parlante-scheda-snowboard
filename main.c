...
int main (int argn, char* argv[]) {
	...
	while(1) {
		...
		if(operator_choise == TELL_ME_DIRECTION) {
			puts("TELL_ME_DIRECTION");
			say_direction();
		}
		if(operator_choise == TELL_ME_ROOLLING) {
			puts("TELL_ME_ROOLLING");
			say_rolling();
		}
		if(operator_choise == TELL_ME_PITCHING) {
			puts("TELL_ME_PITCHING");
			say_pitching();
		}
